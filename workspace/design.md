...
### Methods 
*void send(byte[] bytes, int offset, int length)* : loops over over while each byte hasn't been sent. Moreover, the length of the message sent will be the first byte sent in order for the remote process to know how many bytes it will wait for.

*byte[] receive()* : Returns a message as a whole. After the first byte that describes the length of the message, loops over until the whle message is recieved. Finally it returns it.
